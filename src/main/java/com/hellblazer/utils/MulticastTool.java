...
public class MulticastTool {
	...
	private static final CommandParser cmd = new CommandParser() {
		...
		protected void init() {
			category("Options");
			opt('h', "host").type(String.class).value("233.1.2.30")
					.description("Address of the multicast channel");

			opt('p', "port").type(int.class).value(1966)
			...
			opt('s', "send").type(String.class).description(
					"Optional message to broadcast to the channel");
			opt('r', "rate")
					.type(long.class)
					.value(1000)
					.description(
							"Resend every N milliseconds. Zero sends just once");

			category("Advanced");

			opt("broadcast").type(boolean.class).description(
					"java.net.MulticastSocket#setBroadcast");
			opt("loopback-mode").type(boolean.class).description(
					"java.net.MulticastSocket#setLoopbackMode");
			opt("receive-buffer-size").type(int.class).description(
					"java.net.MulticastSocket#setReceiveBufferSize");
			opt("reuse-address").type(boolean.class).description(
					"java.net.MulticastSocket#setReuseAddress");
			opt("send-buffer-size").type(int.class).description(
					"java.net.MulticastSocket#setSendBufferSize");
			opt("so-timeout").type(int.class).description(
					"java.net.MulticastSocket#setSoTimeout");
			opt("time-to-live").type(int.class).description(
					"java.net.MulticastSocket#setTimeToLive");
			opt("traffic-class").type(int.class).description(
					"java.net.MulticastSocket#setTrafficClass");
		}
	...
	public static void main(String[] array) throws Exception {
		...
		SimpleDateFormat format = new SimpleDateFormat(options.get(
				"date-format", "HH:mm:ss"));

		final String host = options.get("host", "233.1.2.30");
		final int port = options.get("port", 1966);
		...
		if (options.has("reuse-address")) {
			s.setReuseAddress(options.get("reuse-address", false));
		}
		if (options.has("broadcast")) {
			s.setBroadcast(options.get("broadcast", false));
		}
		if (options.has("loopback-mode")) {
			s.setLoopbackMode(options.get("loopback-mode", false));
		}
		if (options.has("send-buffer-size")) {
			s.setSendBufferSize(options.get("send-buffer-size", 0));
		}
		if (options.has("receive-buffer-size")) {
			s.setReceiveBufferSize(options.get("receive-buffer-size", 0));
		}
		if (options.has("so-timeout")) {
			s.setSoTimeout(options.get("so-timeout", 0));
		}
		if (options.has("time-to-live")) {
			s.setTimeToLive(options.get("time-to-live", 0));
		}
		if (options.has("traffic-class")) {
			s.setTrafficClass(options.get("traffic-class", 0));
		}
