 ...
 * a property is not found. If a property is found but its value cannot be
 * parsed as the desired data type, the parent's value is used.
 * <p/>
 * By default this object will log nothing, but if a Log implementation is set
 * the Options class will log three kinds of statements:
 * <p/>
 * - When a property is not found: the property name and default value in use
 * along with all possible values (enums only). Debug level. - When a property
 * is found: the property name and value. Info level. - When a property value
 ...
 * the standard log output and allows us to easily see which values the user has
 * changed from the default. It's rather impossible to diagnose issues without
 * this information.
 * 
 * ENUM SETS:
 * 
 * Properties that accept a Set of enum values automatically accept ALL and NONE
 * in addition to the explicitly created enum items.
 * 
 * Using ALL. This allows users to have an easy way to imply "all" without
 * having to hardcode an the entire list of enum items and protects against the
 * case where that list may grow in the future.
 * 
 * Using NONE. This allows users an alternative to using an empty string when
 * explicitly specifying that none of the options should be used.
 * 
 * In the internal code, this allows us to have these concepts in all enum
 * options without us having to add NONE or ALL enum items explicitly which
 * leads to strange code.
 * 
 * Additionally TRUE is an alias for ALL and FALSE an alias for NONE. This
 * allows options that used to support only true/false values to be further
 * defined in the future without breaking compatibility.
 * 
 * @version $Rev$ $Date$
 */
...
public class Options {
	...
	private final static class NullOptions extends Options {
		...
		protected <T extends Enum<T>> Set<T> getAll(String property,
				Set<T> defaults, Class<T> enumType) {
			if (getLogger().isDebugEnabled()) {
				...
				getLogger().debug(
						"Using default \'" + property + "=" + defaultValues
								+ "\'" + possibleValues);
			}
	...
	public <T extends Enum<T>> T get(String property, T defaultValue) {
		...
		if (defaultValue == null) {
			throw new IllegalArgumentException(
					"Must supply a default for property " + property);
		}
	...
	private void warn(String property, String value) {
		getLogger().warn(
				"Cannot parse supplied value \"" + value + "\" for option \""
						+ property + "\"");
	}
	...
	protected <T extends Enum<T>> Set<T> getAll(String property,
			Set<T> defaultValue, Class<T> enumType) {
		String value = properties.getProperty(property);
