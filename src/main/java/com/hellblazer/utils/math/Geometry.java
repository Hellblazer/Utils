...
public final class Geometry {
     ...
     */
    public static void centerCircle(double xa, double ya, double xb, double yb,
                                    double xc, double yc, double[] po) {
        double acx = xa - xc;
        double bcx = xb - xc;
        double acy = ya - yc;
        double bcy = yb - yc;
     ...
     *            array containing (x,y) coordinates of center.
     */
    public static void centerCircle(double[] pa, double[] pb, double[] pc,
                                    double[] po) {
     ...
     *            array containing (x,y) coordinates of center.
     */
    public static void centerCircle(float xa, float ya, float xb, float yb,
                                    float xc, float yc, float[] po) {
        double acx = xa - xc;
        double bcx = xb - xc;
        double acy = ya - yc;
        double bcy = yb - yc;
     ...
     *            array containing (x,y) coordinates of center.
     */
    public static void centerCircle(float[] pa, float[] pb, float[] pc,
                                    float[] po) {
        centerCircle(pa[0], pa[1], pb[0], pb[1], pc[0], pc[1], po);
    }
     ...
     */
    public static void centerCircle3D(double xa, double ya, double za,
        ...
        po[0] = xc
                + scale
                * ((acs * bcy - bcs * acy) * abz - (acs * bcz - bcs * acz)
                                                   * aby);
        po[1] = yc
                + scale
                * ((acs * bcz - bcs * acz) * abx - (acs * bcx - bcs * acx)
                                                   * abz);
        po[2] = zc
                + scale
                * ((acs * bcx - bcs * acx) * aby - (acs * bcy - bcs * acy)
                                                   * abx);
    }
     ...
     *            array containing (x,y,z) coordinates of center.
     */
    public static void centerSphere(double xa, double ya, double za, double xb,
                                    double yb, double zb, double xc, double yc,
                                    double zc, double xd, double yd, double zd,
                                    double[] po) {
        double adx = xa - xd;
        double bdx = xb - xd;
        double cdx = xc - xd;
        ...
        double scale = 0.5 / leftOfPlane(xa, ya, za, xb, yb, zb, xc, yc, zc,
                                         xd, yd, zd);
        po[0] = xd
                + scale
                * (ads * (bdy * cdz - cdy * bdz) + bds
                   * (cdy * adz - ady * cdz) + cds * (ady * bdz - bdy * adz));
        po[1] = yd
                + scale
                * (ads * (bdz * cdx - cdz * bdx) + bds
                   * (cdz * adx - adz * cdx) + cds * (adz * bdx - bdz * adx));
        po[2] = zd
                + scale
                * (ads * (bdx * cdy - cdx * bdy) + bds
                   * (cdx * ady - adx * cdy) + cds * (adx * bdy - bdx * ady));
    }
     ...
     *            array containing (x,y,z) coordinates of center.
     */
    public static void centerSphere(double[] pa, double[] pb, double[] pc,
                                    double[] pd, double[] po) {
        centerSphere(pa[0], pa[1], pa[2], pb[0], pb[1], pb[2], pc[0], pc[1],
                     pc[2], pd[0], pd[1], pd[2], po);
    }
     ...
     *            array containing (x,y,z) coordinates of center.
     */
    public static void centerSphere(float xa, float ya, float za, float xb,
                                    float yb, float zb, float xc, float yc,
        ...
        double scale = 0.5 / leftOfPlane(xa, ya, za, xb, yb, zb, xc, yc, zc,
                                         xd, yd, zd);
        po[0] = xd
                + (float) (scale * (ads * (bdy * cdz - cdy * bdz) + bds
                                    * (cdy * adz - ady * cdz) + cds
                                                                * (ady * bdz - bdy
                                                                               * adz)));
        po[1] = yd
                + (float) (scale * (ads * (bdz * cdx - cdz * bdx) + bds
                                    * (cdz * adx - adz * cdx) + cds
                                                                * (adz * bdx - bdz
                                                                               * adx)));
        po[2] = zd
                + (float) (scale * (ads * (bdx * cdy - cdx * bdy) + bds
                                    * (cdx * ady - adx * cdy) + cds
                                                                * (adx * bdy - bdx
                                                                               * ady)));
    }
     ...
     *            array containing (x,y,z) coordinates of center.
     */
    public static void centerSphere(float[] pa, float[] pb, float[] pc,
                                    float[] pd, float[] po) {
        centerSphere(pa[0], pa[1], pa[2], pb[0], pb[1], pb[2], pc[0], pc[1],
                     pc[2], pd[0], pd[1], pd[2], po);
    }
     ...
     *         zero, otherwise.
     */
    public static double inCircle(double xa, double ya, double xb, double yb,
     ...
     *         zero, otherwise.
     */
    public static double inCircle(double[] pa, double[] pb, double[] pc,
                                  double[] pd) {
     ...
     *         zero, otherwise.
     */
    public static double inCircleFast(double xa, double ya, double xb,
     ...
     *         zero, otherwise.
     */
    public static double inCircleFast(double[] pa, double[] pb, double[] pc,
                                      double[] pd) {
        return inCircleFast(pa[0], pa[1], pb[0], pb[1], pc[0], pc[1], pd[0],
                            pd[1]);
    }
     ...
     *         zero, otherwise.
     */
    public static double inCircleFast(float[] pa, float[] pb, float[] pc,
                                      float[] pd) {
        return inCircleFast(pa[0], pa[1], pb[0], pb[1], pc[0], pc[1], pd[0],
                            pd[1]);
    }
     ...
     * efficient.
     */
    public static double inOrthoSphere(double xa, double ya, double za,
                                       double wa, double xb, double yb,
                                       double zb, double wb, double xc,
                                       double yc, double zc, double wc,
                                       double xd, double yd, double zd,
                                       double wd, double xe, double ye,
                                       double ze, double we) {
        double aex = xa - xe;
        double bex = xb - xe;
        double cex = xc - xe;
        ...
        double permanent = ((cexdey + dexcey) * bez + (dexbey + bexdey) * cez + (bexcey + cexbey)
                                                                                * dez)
                           * (alift + aew)
                           + ((dexaey + aexdey) * cez + (aexcey + cexaey) * dez + (cexdey + dexcey)
                                                                                  * aez)
                           * (blift + bew)
                           + ((aexbey + bexaey) * dez + (bexdey + dexbey) * aez + (dexaey + aexdey)
                                                                                  * bez)
                           * (clift + cew)
                           + ((bexcey + cexbey) * aez + (cexaey + aexcey) * bez + (aexbey + bexaey)
                                                                                  * cez)
                           * (dlift + dew);
        ...
        return inOrthoSphereExact(xa, ya, za, wa, xb, yb, zb, wb, xc, yc, zc,
                                  wc, xd, yd, zd, wd, xe, ye, ze, we);
    }
     ...
     * positive number.
     */
    public static double inOrthoSphere(double[] pa, double[] pb, double[] pc,
                                       double[] pd, double[] pe) {
        return inOrthoSphere(pa[0], pa[1], pa[2], pa[3], pb[0], pb[1], pb[2],
                             pb[3], pc[0], pc[1], pc[2], pc[3], pd[0], pd[1],
                             pd[2], pd[3], pe[0], pe[1], pe[2], pe[3]);
    }
     ...
     * positive number.
     */
    public static double inOrthoSphere(float[] pa, float[] pb, float[] pc,
                                       float[] pd, float[] pe) {
        return inOrthoSphere(pa[0], pa[1], pa[2], pa[3], pb[0], pb[1], pb[2],
                             pb[3], pc[0], pc[1], pc[2], pc[3], pd[0], pd[1],
                             pd[2], pd[3], pe[0], pe[1], pe[2], pe[3]);
     ...
     * <em>Note: this fast method may return an incorrect result.</em>
     */
    public static double inOrthoSphereFast(double xa, double ya, double za,
                                           double wa, double xb, double yb,
                                           double zb, double wb, double xc,
                                           double yc, double zc, double wc,
                                           double xd, double yd, double zd,
     ...
     * <em>Note: this fast method may return an incorrect result.</em>
     */
    public static double inOrthoSphereFast(double[] pa, double[] pb,
                                           double[] pc, double[] pd, double[] pe) {
        return inOrthoSphereFast(pa[0], pa[1], pa[2], pa[3], pb[0], pb[1],
                                 pb[2], pb[3], pc[0], pc[1], pc[2], pc[3],
                                 pd[0], pd[1], pd[2], pd[3], pe[0], pe[1],
                                 pe[2], pe[3]);
    }
     ...
     * <em>Note: this fast method may return an incorrect result.</em>
     */
    public static double inOrthoSphereFast(float[] pa, float[] pb, float[] pc,
                                           float[] pd, float[] pe) {
        return inOrthoSphereFast(pa[0], pa[1], pa[2], pa[3], pb[0], pb[1],
                                 pb[2], pb[3], pc[0], pc[1], pc[2], pc[3],
                                 pd[0], pd[1], pd[2], pd[3], pe[0], pe[1],
     ...
     *         zero, otherwise.
     */
    public static double inSphere(double xa, double ya, double za, double xb,
                                  double yb, double zb, double xc, double yc,
                                  double zc, double xd, double yd, double zd,
        ...
        return inSphereExact(xa, ya, za, xb, yb, zb, xc, yc, zc, xd, yd, zd,
                             xe, ye, ze);
    }
     ...
     *         zero, otherwise.
     */
    public static double inSphere(double[] pa, double[] pb, double[] pc,
                                  double[] pd, double[] pe) {
     ...
     *         zero, otherwise.
     */
    public static double inSphere(float[] pa, float[] pb, float[] pc,
                                  float[] pd, float[] pe) {
        return inSphere(pa[0], pa[1], pa[2], pb[0], pb[1], pb[2], pc[0], pc[1],
                        pc[2], pd[0], pd[1], pd[2], pe[0], pe[1], pe[2]);
    }
     ...
     *         zero, otherwise.
     */
    public static double inSphereFast(double xa, double ya, double za,
                                      double xb, double yb, double zb,
                                      double xc, double yc, double zc,
                                      double xd, double yd, double zd,
                                      double xe, double ye, double ze) {
     ...
     *         zero, otherwise.
     */
    public static double inSphereFast(double[] pa, double[] pb, double[] pc,
                                      double[] pd, double[] pe) {
        return inSphereFast(pa[0], pa[1], pa[2], pb[0], pb[1], pb[2], pc[0],
                            pc[1], pc[2], pd[0], pd[1], pd[2], pe[0], pe[1],
                            pe[2]);
    }
     ...
     *         zero, otherwise.
     */
    public static double inSphereFast(float[] pa, float[] pb, float[] pc,
                                      float[] pd, float[] pe) {
        return inSphereFast(pa[0], pa[1], pa[2], pb[0], pb[1], pb[2], pc[0],
                            pc[1], pc[2], pd[0], pd[1], pd[2], pe[0], pe[1],
                            pe[2]);
     ...
     *         otherwise.
     */
    public static double leftOfLine(double xa, double ya, double xb, double yb,
                                    double xc, double yc) {
     ...
     *         otherwise.
     */
    public static double leftOfLineFast(double xa, double ya, double xb,
                                        double yb, double xc, double yc) {
        double acx = xa - xc;
        double bcx = xb - xc;
        double acy = ya - yc;
     ...
     *         otherwise.
     */
    public static double leftOfPlane(double xa, double ya, double za,
                                     double xb, double yb, double zb,
                                     double xc, double yc, double zc,
                                     double xd, double yd, double zd) {
        double adx = xa - xd;
        ...
        double det = adz * (bdxcdy - cdxbdy) + bdz * (cdxady - adxcdy) + cdz
                     * (adxbdy - bdxady);
        ...
        double permanent = (bdxcdy + cdxbdy) * adz + (cdxady + adxcdy) * bdz
                           + (adxbdy + bdxady) * cdz;
     ...
     *         otherwise.
     */
    public static double leftOfPlane(double[] pa, double[] pb, double[] pc,
                                     double[] pd) {
        return leftOfPlane(pa[0], pa[1], pa[2], pb[0], pb[1], pb[2], pc[0],
     ...
     *         otherwise.
     */
    public static double leftOfPlane(float[] pa, float[] pb, float[] pc,
                                     float[] pd) {
        return leftOfPlane(pa[0], pa[1], pa[2], pb[0], pb[1], pb[2], pc[0],
                           pc[1], pc[2], pd[0], pd[1], pd[2]);
    }
     ...
     *         otherwise.
     */
    public static double leftOfPlaneFast(double[] pa, double[] pb, double[] pc,
                                         double[] pd) {
        return leftOfPlaneFast(pa[0], pa[1], pa[2], pb[0], pb[1], pb[2], pc[0],
                               pc[1], pc[2], pd[0], pd[1], pd[2]);
    }
     ...
     *         otherwise.
     */
    public static double leftOfPlaneFast(float[] pa, float[] pb, float[] pc,
                                         float[] pd) {
        return leftOfPlaneFast(pa[0], pa[1], pa[2], pb[0], pb[1], pb[2], pc[0],
                               pc[1], pc[2], pd[0], pd[1], pd[2]);
    }
     ...
     * aliased with e or f.
     */
    private static int expansionSumZeroElimFast(int elen, double[] e, int flen,
                                                double[] f, double[] h) {
        double q, qnew, hh;
        Two t = new Two();
        double enow = e[0];
     ...
     * will be reversed.
     */
    private static double inCircleExact(double xa, double ya, double xb,
                                        double yb, double xc, double yc,
                                        double xd, double yd) {
        Two t = new Two();
        twoDiff(xa, xd, t);
     ...
     */
    private static double inOrthoSphereExact(double xa, double ya, double za,
                                             double wa, double xb, double yb,
                                             double zb, double wb, double xc,
                                             double yc, double zc, double wc,
                                             double xd, double yd, double zd,
    ...
    */
     ...
     */
    private static double inSphereExact(double xa, double ya, double za,
                                        double xb, double yb, double zb,
                                        double xc, double yc, double zc,
                                        double xd, double yd, double zd,
                                        double xe, double ye, double ze) {
    ...
    */
     ...
     */
    private static double leftOfLineExact(double xa, double ya, double xb,
                                          double yb, double xc, double yc) {
        Two t = new Two();
        twoDiff(xa, xc, t);
        double acx = t.x;
     ...
     */
    private static double leftOfPlaneExact(double xa, double ya, double za,
                                           double xb, double yb, double zb,
                                           double xc, double yc, double zc,
                                           double xd, double yd, double zd) {
        Two t = new Two();
     ...
     */
    private strictfp static void twoProduct1Presplit(double a, double b,
     ...
     * product in x and the error in y.
     */
    private strictfp static void twoProduct2Presplit(double a, double ahi,
                                                     double alo, double b,
                                                     double bhi, double blo,
                                                     Two t) {
        t.x = a * b;
     ...
     */
    private strictfp static void twoTwoProduct(double a1, double a0, double b1,
                                               double b0, double[] x) {
